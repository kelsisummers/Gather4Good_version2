...
export const EventCard = (props) => {
    ...
    const attendeeBadge = (attendees.includes(userId)) && (userId !== organizerId) ? (<Badge style={{padding: '5px', color: 'white', opacity: '.5', borderRadius: '2px'}}>Attendee</Badge>) : (<div/>)
            ...
            <Row>
              <Panel.Heading>
                ...
                <Col md={2} style={{marginTop: 21, paddingRight:0}}>
                  <Badge style={{ padding: '10px', borderRadius: 2, backgroundColor: '#00b9b4', fontSize: 14, color: 'white', opacity: '.7', float: 'right'}}>{props.data.cause.name}</Badge>
                </Col>
              </Panel.Heading>
            </Row>
            <Row>
